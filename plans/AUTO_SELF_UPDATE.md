...
| 27 | Multi-path self-heal stress test | HIGH | Exercise all 3 self-heal trigger paths in one PR: ci-failure, review-findings (criticals), review-findings (all-findings). 4 intentional bugs trigger cascade. Procedure: `tests/e2e/self-heal-ci-failure.md` § "Multi-Path Stress Test". | DONE — PR #55. Attempt 1 (ci-failure) removed broken test. Attempt 2 (review-findings, all-findings) fixed suggestions. Review correctly identified intentional bugs as non-critical. Full cascade proven. |
| 28 | Validate auto PR Review re-trigger | MED | PR #55 found that self-heal doesn't re-trigger PR Review after fix (GITHUB_TOKEN anti-loop). Fix merged in PR #55: `ci-self-heal.yml` now adds `needs-review` label after push. Validate on next PR that the full cascade (CI fail → fix → CI pass → review → findings → fix → clean) works automatically without manual label. | PLANNED — first PR after #55 merges |
