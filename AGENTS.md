...
This repo recommends three setup lanes — Setup A (Claude Premium: Fable 5 advisor, Opus 4.6 max driver, GPT-5.5 xhigh reviewer + Fable escalation), Setup B (Claude Saver: OpusPlan + Opus advisor + Sonnet driver + GPT-5.5 reviewer), and Setup C (Claude Lite: Sonnet driver, no reviewer). See [`AI_SETUP_LANES.md`](AI_SETUP_LANES.md) for the full pick list.
