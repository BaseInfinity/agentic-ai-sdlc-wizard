...
test_instructions_hook_cwd_walkup() {
    ...
    # doesn't poison the staleness check (real-world bug: a stale 1.73.0 cache
    # makes a 1.43.0 install look "30 releases behind" and the loud nudge fires).
    output=$(cd "$tmpdir/project/src" && PATH="$tmpdir/bin:$PATH" HOME="$tmpdir" CLAUDE_PROJECT_DIR="" "$HOOKS_DIR/instructions-loaded-check.sh" 2>/dev/null)
    rm -rf "$tmpdir"
