...
| `codex-gate-check.sh` | Before `git commit` (Bash) | Blocks (exit 2) commits without a REVIEWED/CERTIFIED `.reviews/handoff.json`, or with a stale (`commit_sha` mismatch) certification. An in-flight `PENDING_RECHECK` round commits on the branch it declares in `branch` — missing field, detached HEAD, branch mismatch, a literal `main`/`master`, or a relocation flag all still block (#533) |
