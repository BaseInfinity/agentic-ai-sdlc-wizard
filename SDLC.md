...
- **Merging to main does NOT trigger npm publish.** You must create and push a git tag (`git tag v<version> && git push origin v<version>`) to trigger `release.yml`. Verify with `npm view agentic-sdlc-wizard version`. The afterhours pre-push hook blocks tag pushes during work hours — use `AFTERHOURS_SKIP=1`. For remote branch deletions, use the GitHub API (`gh api -X DELETE repos/.../git/refs/heads/<branch>`) instead of `git push --delete` to bypass the hook entirely. (Source: v1.82.0 incident, 2026-06-11)
