...
> **Updated 2026-07-25 (#478) — the denylist is now TIERED, and approval is no longer the same lever as bypass.** `scripts/merge-pr.sh` splits matched paths into two tiers. **HARD** — `.github/workflows/`, `hooks/`, `.claude/`, the wrapper itself, both `skills/sdlc/SKILL.md` copies, and `CHANGELOG.md` — cannot be acknowledged in the artifact; a human approves them with `./scripts/merge-pr.sh <N> --user-approved "<why>"`. **ACKABLE** — this document — proceeds when the clearance artifact carries a JSON `denylist_ack` naming each matched path with a non-empty reason and `"user_confirmed": true` (parsed with a real JSON parser, fail-closed on malformed/duplicate/wrong-shaped input). **Critically, `--user-approved` disables NOTHING** — the CI `validate` check, the net-test-deletion check, and every clearance-artifact check still run. That distinction exists because the previous design made `MERGE_CLEARANCE_SKIP=1` the only way to say "yes, merge this", so expressing approval required switching off the safety system. `MERGE_CLEARANCE_SKIP=1` survives strictly as an emergency override that skips everything. `skills/sdlc/SKILL.md` is HARD, not ackable, because `.claude/skills/sdlc` is a tracked symlink to it — editing it changes the live loaded skill without producing any `.claude/` path in the diff.
